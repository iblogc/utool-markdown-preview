...
一款为 uTools 打造的高效 Markdown / HTML / SVG 预览插件，支持极速渲染、多窗口管理、Mermaid 绘图以及一键导出功能。
...
- 🚀 **极速渲染**：采用实时解析引擎，确保 Markdown 内容即刻呈现。
- � **剪贴板预览**：支持直接读取剪贴板中的文本内容唤起 uTools 进行预览。
- �📁 **文件一键预览**：支持在资源管理器中复制本地 `.md`、`.html` 或 `.svg` 文件后，直接通过 uTools 唤起插件进行预览。
- 📱 **灵动工具栏**：底部悬浮中心设计，磨砂玻璃质感。采用**分类配色方案**（目录/Markdown/图片/HTML），操作直观且不干扰阅读。
...
- 💾 **多维导出与共享**：
  ...
  - **预览图**：支持将 Markdown、HTML 或 SVG 一键渲染为高清图片并**保存**或**直接复制到剪贴板**，方便在社交软件中分享。
  - **HTML**：支持导出包含完整独立样式的 .html（保留图表与布局）。
- 🔔 **优雅消息提示**：导出/复制操作完成后，顶部显示自定义消息提示（成功/失败），无需依赖系统通知。

## 🛠️ 使用方法

1. **复制内容或文件**：
   - **文本**：在任何地方复制你想预览的 Markdown 源码。
   - **文件**：在资源管理器中复制 `.md`、`.html` 或 `.svg` 后缀的本地文件。
2. **呼出插件**：在 uTools 搜索框输入 `md预览` 或 `markdown预览`。如果是复制的文件，uTools 会自动匹配到本插件。
3. **预览管理**：
   - 第一次进入自动开启预览。
